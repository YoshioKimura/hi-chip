...
if(isset($_POST['postId'])){
    ...
    try{
        ...
        $sql = 'SELECT * FROM likes WHERE praise_id = :praise_id AND liker_user_id = :user_id';
        ...
        if(!empty($resultCount)){
            // レコードを削除する
            $sql = 'DELETE FROM likes WHERE praise_id = :praise_id AND liker_user_id = :user_id';
            $data = array(':praise_id' => $praise_id, ':user_id' => $_SESSION['user_id']);
            // クエリ実行
            $stmt = queryPost($pdo, $sql, $data);
            ...
            // レコードを挿入する
            $sql = 'INSERT INTO likes (praise_id, liker_user_id, created_at) VALUES (:praise_id, :user_id, :date)';
